    ...
    loop {
            ...
            .await
        {
            Ok(db_last_reprocess) => {
                info!(target: "reprocess_log", "CHROMIUM: Last reprocessing was {}", db_last_reprocess.created_at);
                last_reprocess = Some(db_last_reprocess);
            }
            Err(e) => {
                tracing::error!(target: "reprocess_log", "CHROMIUM: Failed to get last reprocessing {:?}", e);
            }
        }
        ...
        {
            info!(target: "reprocess_log", "CHROMIUM: Last reprocessing was less than 3 days ago, waiting then retrying");
        ...
        if check_current_reprocessing.is_some() {
            info!(target: "reprocess_log", "CHROMIUM: Last reprocessing was not finished, waiting then retrying");
            ...
            while let Err(_) = app_database
                ...
                .await
            {
                tracing::error!(target: "reprocess_log", "Failed to add chromium reprocessing to db. Retrying...");
        ...
        loop {
                ...
                .await
            {
                ...
                Err(_) => {
                    tracing::error!(target: "reprocess_log", "CHROMIUM: Failed to get current reprocessing. Retrying...");
                    tokio::time::sleep(Duration::from_secs(1)).await;
                }
            }
        }
        ...
        info!(target: "reprocess_log", "CHROMIUM: Starting reprocessing system");
        // Init ata
        ...
        if let Ok(Some(_ata)) = rpc_client.get_token_account(&token_account_pubkey).await {
            info!(target: "reprocess_log", "CHROMIUM: Chromium ATA already exists");
        } else {
            info!(target: "reprocess_log", "CHROMIUM: Generating Chromium ATA");
            ...
            {
                Ok(_) => {
                    info!(target: "reprocess_log", "CHROMIUM: Chromium ATA created");
                }
                Err(e) => {
                    tracing::error!(target: "reprocess_log", "CHROMIUM: Exiting Chromium reprocessing system: Failed to create Chromium ATA {}", e);
                    continue;
                }
            }
        }
        ...
        info!(target: "reprocess_log", "CHROMIUM: Reprocessor found, moving with next steps");

        let target_slot = reprocessor.unwrap().slot;

        info!(target: "reprocess_log", "CHROMIUM: Waiting for slot {}...", target_slot);
        ...
        loop {
            match rpc_client.get_slot().await {
                ...
                Err(e) => {
                    tracing::error!(target: "reprocess_log", "CHROMIUM: Failed to get current slot {}...", e);
                    tokio::time::sleep(Duration::from_secs(400)).await;
                }
            }
        }
        info!(target: "reprocess_log", "CHROMIUM: Waiting a bit more for good measure 1");
        tokio::time::sleep(Duration::from_millis(25000)).await;
        info!(target: "reprocess_log", "CHROMIUM: Waiting a bit more for good measure 2");
        tokio::time::sleep(Duration::from_millis(25000)).await;
        info!(target: "reprocess_log", "CHROMIUM: Waiting a bit more for good measure 3");
        tokio::time::sleep(Duration::from_millis(25000)).await;
        info!(target: "reprocess_log", "CHROMIUM: Waiting a bit more for good measure 4");
        tokio::time::sleep(Duration::from_millis(25000)).await;
        let token_account = rpc_client
            .get_token_account(&token_account_pubkey)
            .await
        ...
        let final_balance = (token_account.unwrap().token_amount.ui_amount.unwrap()
            * 10f64.powf(COAL_TOKEN_DECIMALS as f64)) as u64;
        info!(target: "reprocess_log", "CHROMIUM: initial_balance {} final_balance {}",initial_balance, final_balance);
        let mut full_reprocessed_amount = final_balance - initial_balance;
        if full_reprocessed_amount <= 0 {
            tracing::error!(target: "reprocess_log", "CHROMIUM: Chromium reprocessing system: Got 0 reprocessed amount");
            full_reprocessed_amount = 0;
        }
        info!(target: "reprocess_log", "CHROMIUM: Reprocessed {} Chromium", full_reprocessed_amount);
        ...
        tracing::info!(target: "reprocess_log", "CHROMIUM: Inserting commissions earning");
        while let Err(_) = app_database
            .add_new_earnings_extra_resources_batch(commissions_earning.clone())
            .await
        {
            tracing::error!(target: "reprocess_log", "CHROMIUM: Failed to add commissions earning... retrying...");
            tokio::time::sleep(Duration::from_millis(500)).await;
        }
        tracing::info!(target: "reprocess_log", "CHROMIUM: Inserted commissions earning");
        ...
        if commission_rewards.len() > 0 {
            for batch in commission_rewards.chunks(100) {
                while let Err(_) = app_database.update_rewards(batch.to_vec()).await {
                    tracing::error!(target: "reprocess_log", "CHROMIUM: Failed to add new commissions rewards to db. Retrying...");
                    tokio::time::sleep(Duration::from_millis(500)).await;
                }
        ...
        tracing::info!(target: "reprocess_log", "CHROMIUM: Inserted commissions rewards");
        ...
        loop {
                ...
                .await
            {
                ...
                Err(_) => {
                    tracing::error!(target: "reprocess_log", "CHROMIUM: Failed to get user submissions. Retrying...");
                    tokio::time::sleep(Duration::from_secs(1)).await;
                }
            }
        }
        ...
        info!(target: "reprocess_log", "CHROMIUM: Total Miners: {}, total miner perc: {}", miner_stats_perc.len(), total_miners_perc);
        ...
        if miners_earnings.len() > 0 {
            for batch in miners_earnings.chunks(batch_size) {
                while let Err(_) = app_database
                    ...
                    .await
                {
                    tracing::error!(target: "reprocess_log", "CHROMIUM: Failed to add new earnings batch to db. Retrying...");
                    tokio::time::sleep(Duration::from_millis(500)).await;
                }
                tokio::time::sleep(Duration::from_millis(200)).await;
            }
            info!(target: "reprocess_log", "CHROMIUM: Successfully added earnings batch");
        }
        ...
        if miners_rewards.len() > 0 {
            for batch in miners_rewards.chunks(batch_size) {
                while let Err(_) = app_database.update_rewards(batch.to_vec()).await {
                    tracing::error!(target: "reprocess_log", "CHROMIUM: Failed to add new rewards batch to db. Retrying...");
                    tokio::time::sleep(Duration::from_millis(500)).await;
                }
                tokio::time::sleep(Duration::from_millis(200)).await;
            }
            info!(target: "reprocess_log", "CHROMIUM: Successfully added rewards batch");
        }
        ...
        while let Err(_) = app_database
            ...
            .await
        {
            tracing::error!(target: "reprocess_log", "CHROMIUM: Failed to finish chromium reprocessing to db. Retrying...");
            tokio::time::sleep(Duration::from_secs(1)).await;
        }
    }
}
